...
This plan turns the MVP scope in `.squad/specification/namma-nyaya.md` (rental agreements + offer letters for migrant tech professionals in Bengaluru, with multilingual OCR ingestion, three-level explanation, red-flag detection, compare/diff, cited Q&A over a small curated source set, an IPC/CrPC/Evidence Act → BNS/BNSS/BSA mapping tool, a 2-3 playbook navigator, and baseline responsible-AI guardrails) into concrete, buildable technical approaches for a time-boxed hackathon. Because the feature set is heavily LLM/RAG- and multilingual-OCR-dependent, this plan focuses in particular on backend framework, frontend shape, OCR/ASR provider, LLM/model tiering, retrieval approach for the small curated source set, and a deployment target that is realistic to stand up during a hackathon. No implementation is included — this is options and trade-offs only, with a recommendation and an explicit list of decisions the user/team still needs to make.
...
Two overall end-to-end approaches are presented, followed by per-dimension options (OCR, model, retrieval) that apply to both, since those choices are largely independent of which overall approach is picked.

### Approach 1 — Fast-build, Python-only stack (Streamlit/Gradio front end)
...
- **Cons**: Visibly less polished than a custom web UI — judges will see a "notebook-style" app rather than a product; harder to do precise clause-highlight overlays directly on a scanned document image (Streamlit/Gradio are not built for pixel-level annotation); harder to extend into a "real" product later (would likely need a rewrite of the front end).
- **Complexity**: Low.

### Approach 2 — Full-stack web app (Python backend + React/Next.js frontend)

- **Backend**: Python + FastAPI, handling upload intake, OCR orchestration, PII masking, classification, the RAG/Q&A pipeline, red-flag/compare logic, and the old-to-new law mapping lookup. FastAPI is a strong fit because it is async-friendly (useful for chaining OCR → LLM calls without blocking), has first-class OpenAPI docs (useful for a demo judge or teammate to inspect), and Python remains the natural home for OCR/Indian-language libraries.
- **Frontend**: Next.js/React, giving a real side-by-side document viewer (original scan/PDF on one side, clause-by-clause explanation with color-coded highlights on the other), a chat panel for Q&A with inline citations, and a language toggle — the kind of visual demo that reads as a "product" rather than a script.
- **Pros**: Much stronger demo polish — a side-by-side annotated document view is a strong visual differentiator for judges and closely matches the spec's description of the explanation/red-flag UX; clean separation between orchestration logic and UI; closer to what a real post-hackathon product would need, so less gets thrown away.
- **Cons**: Two codebases/deploys to wire up (CORS, shared types/contracts, two hosting targets); more hackathon time spent on plumbing before any feature work shows results; higher risk of running out of time before the demo is fully wired end-to-end.
- **Complexity**: Medium-High.

### Backend language alternative worth naming and rejecting: Node/TypeScript full-stack (Next.js API routes)

- A single Next.js app (API routes as the backend, same repo as the frontend) using a JS-native LLM orchestration layer (e.g. Vercel AI SDK or LangChain.js) is a real alternative to a separate Python backend: one language, one repo, one Vercel deploy.
- **Rejected as the primary recommendation** because: (a) the OCR/Indian-language tooling landscape (Bhashini reference clients, AI4Bharat models, most Indian-language OCR sample code) is overwhelmingly Python-first; (b) OCR + multi-step LLM chaining for a full document (OCR → classify → explain → red-flag → cite) can run long, and Vercel's serverless function execution-time limits (short on free/hobby tiers) are a real risk for this specific pipeline versus a long-running Python process on Render/Railway. It remains a legitimate option if the team is far more fluent in TypeScript than Python — see Decisions needed.

### Model choice (applies to either approach)

The pipeline has at least three distinct LLM-shaped jobs with different quality/latency/cost needs, so a **tiered model strategy** is recommended rather than one model for everything:

- **Document classification, PII-pattern triage, high-stakes trigger detection** — cheap/fast tasks that don't need top-tier reasoning. A smaller, fast model fits: Claude Haiku (current generation) or GPT-4o-mini class. Keeps per-request cost and latency low, which matters for a live, repeated demo.
- **Explanation generation, red-flag reasoning, cited Q&A synthesis** — this is where citation discipline, refusal behavior ("I don't know" when ungrounded), and legal-register reasoning quality matter most, and where a wrong or fabricated citation is most damaging to the product's credibility. A flagship-tier model is worth the cost here: Claude Sonnet (current generation, e.g. Sonnet 4.x) is a strong fit given its instruction-following and grounded-citation behavior; GPT-4o/GPT-4.1-class models are a comparable alternative if the team has OpenAI credits instead. Reaching for the absolute largest/most expensive tier (e.g. Claude Opus, GPT-4.5/o1-class) is probably unnecessary for MVP scope and adds latency/cost without a clear demo-time benefit, but is worth keeping in mind if red-flag/citation quality turns out to be the weak link in testing.
- **Embeddings for retrieval** — neither Anthropic nor a "pick one" default is obvious here since Anthropic does not currently offer a first-party embeddings API. Options: Voyage AI embeddings (Anthropic's recommended embeddings partner), OpenAI's `text-embedding-3` family, Cohere's multilingual embeddings, or an open-source multilingual model such as BGE-M3 run locally. This matters more than usual here because documents may be in Kannada while the curated legal source set is in English — a **multilingual-aware embedding model** (Cohere multilingual, BGE-M3) or an explicit "translate to English before retrieval" step is needed to avoid silently poor cross-lingual retrieval.

Note: model pricing/availability shifts frequently; the specific tier names above reflect current (as of this writing) offerings and should be re-checked against each provider's live docs before committing, especially if the hackathon provides sponsor API credits for a specific provider (see Decisions needed).

### Retrieval / vector store approach for the curated source set

Given the MVP explicitly scopes to "a small, hand-curated source set" (a handful of statutes/sections relevant to rental agreements and offer letters), two viable approaches:

- **Approach A — Lightweight embedded vector store.** Chroma or FAISS running embedded in the backend process (no separate hosted service to stand up), populated once at startup from the curated source documents. Pros: still demonstrates a "real" RAG architecture, no extra infra to deploy, free. Cons: still requires picking and wiring an embeddings model (see above), and is arguably more infrastructure than the curated set's size actually needs.
- **Approach B — Context-stuffing, no vector store.** Because the curated MVP source set is small (a handful of statutes/sections), it may fit entirely within a single flagship-model context window. In that case, skip embeddings/retrieval entirely and pass the full curated source text plus the document's clauses directly into the prompt, letting the model cite directly from the supplied text. Pros: removes an entire moving part (embeddings model choice, vector DB, retrieval-quality tuning) and arguably improves citation reliability for a small corpus since nothing is missed by an imperfect top-k retrieval step; simpler to build and debug within a hackathon window. Cons: doesn't scale to the "full production-grade retrieval pipeline over India Code + Karnataka Gazette + Indian Kanoon" the spec names as full-vision scope, so it is an MVP-only shortcut, not the end-state architecture.
- **Recommendation**: start with Approach B (context-stuffing) for MVP speed and citation reliability, and treat "swap in a real vector store" as the known, well-understood next step for post-hackathon scope-up — this matches the spec's own MVP/stretch split (small curated set now, production retrieval later).

### OCR/ASR options for Kannada + English ingestion

- **Bhashini APIs** (Govt. of India's National Language Translation Mission) — purpose-built for Indian languages including Kannada, free to use; but documentation, uptime, and latency consistency are known to be uneven, which is a real risk for a live demo. Worth a very early spike, not a default assumption.
- **AI4Bharat models** (e.g. IndicOCR/document layout models) — open-source, self-hostable, strong Indian-language research pedigree; but self-hosting adds GPU/infra setup time the hackathon likely can't afford, making this a better fit for a post-hackathon "real" version than for the MVP demo.
- **Sarvam AI** — commercial Indian-language API provider (STT/OCR/translation) aimed at production reliability; faster to integrate than self-hosting AI4Bharat, but is a paid/commercial API — worth checking for hackathon credits/free tier before committing.
- **Google Cloud Vision / Document AI** — general-purpose OCR with solid Kannada script support, mature SDKs, extensive documentation, and fast integration time; not purpose-built for Indian legal-document layouts or code-mixed Kannada/English text, but the most reliable, lowest-integration-risk option for a hackathon timeline.
- **Recommendation**: use Google Cloud Vision/Document AI as the primary OCR engine for the MVP demo given integration speed and reliability, and treat Bhashini/Sarvam AI as a stretch spike (per the spec's own "OCR quality for Kannada" open risk) if time allows — the spec's own open-questions section already flags this exact trade-off as unresolved and needing an early test.

### Deployment target

- **Split deploy (recommended)**: FastAPI backend on Render or Railway (both offer fast, free-tier-friendly deploys with no serverless execution-time ceiling, which matters for chained OCR→LLM calls) + Next.js frontend on Vercel. Fast to stand up, generous free tiers, no surprise timeouts on longer-running requests.
- **Single deploy alternative**: If the Node/Next.js-only backend approach is chosen instead, a single Vercel project can host both frontend and API routes — simplest possible deploy story, but reintroduces the serverless execution-time risk noted above for the OCR-heavy pipeline.
- **Local-only fallback**: Running entirely on localhost for the live demo (no public deploy) is the lowest-infra-risk option for demo day itself, but leaves nothing shareable/persistent afterward for judges to revisit, and most hackathons expect a live link or recorded demo of a hosted instance.
- **Recommendation**: split deploy (Render/Railway + Vercel) as the primary target, with a local fallback rehearsed in case of live demo-network issues.

## Recommended approach

**Approach 2 (Python/FastAPI backend + Next.js/React frontend), Google Cloud Vision/Document AI for OCR, a tiered Claude model strategy (Haiku for cheap sub-tasks, Sonnet for explanation/red-flag/Q&A synthesis) or GPT-4o/4.1-class equivalents if OpenAI credits are the team's actual constraint, context-stuffing instead of a vector store for the small MVP source set, deployed as Render/Railway (backend) + Vercel (frontend).**

Rationale: the spec's differentiators (side-by-side compare/diff, clause-level highlighting, cited Q&A) are inherently visual and benefit from a real document-viewer UI rather than a notebook-style app — this is worth the extra plumbing time given it directly demonstrates the hyperlocal, clause-level value proposition to judges. Python remains the natural home for OCR/Indian-language work regardless of frontend choice. Context-stuffing over the small curated source set removes a whole category of RAG-tuning risk that a hackathon timeline can't afford, while still leaving a clean, well-understood path to a real vector-store retrieval pipeline post-hackathon.
...
1. **LLM provider/model**: Does the team have sponsor-provided API credits for a specific provider (Anthropic, OpenAI, Google) for this hackathon, and is there a hard cost ceiling for the live demo? This should drive the actual model pick more than any abstract quality comparison.
2. **OCR/ASR provider for Kannada**: Bhashini vs. AI4Bharat (self-hosted) vs. Sarvam AI vs. Google Cloud Vision/Document AI — has anyone already spiked any of these against real, code-mixed Kannada/English rental agreements or offer letters? The spec itself flags this as an unresolved, untested risk.
3. **Team stack familiarity**: Is the team already stronger in Python/FastAPI or Node/TypeScript/Next.js? Given the time-box, existing fluency should weigh heavily against the "ideal" stack recommended here.
4. **Frontend polish vs. build speed**: Is a custom side-by-side document-viewer with clause highlights worth the extra build time (Approach 2), or is a Streamlit/Gradio demo (Approach 1) acceptable for the judging criteria being used?
5. **Retrieval architecture**: Given the MVP source set is small, is context-stuffing (no vector store) an acceptable MVP shortcut, or does the team want a "real" vector-store demo (Chroma/FAISS) even at MVP stage — and if so, which embeddings model, given the cross-lingual (Kannada document / English statute) retrieval concern?
6. **Deployment budget/target**: Render/Railway/Fly.io/Vercel, or another platform the team already has accounts/credits for — is there a cost ceiling, and does the demo need to stay live after judging or only during it?
7. **Design-for-future-WhatsApp-bot now?**: Should the MVP architecture be built with a clean, channel-agnostic core API (so a WhatsApp bot could be bolted on later without a rewrite), even though the WhatsApp bot itself is explicitly out of MVP scope — or should the team optimize purely for the web demo without that constraint?
8. **PII masking placement relative to external API calls**: Since documents will be sent to third-party OCR and LLM APIs, should Aadhaar/PAN masking happen on the raw image/OCR output before any external LLM call (stronger privacy guarantee, more engineering work), or only before storage (simpler, but exposes raw PII to the LLM provider in-flight)? This also implies confirming the chosen OCR/LLM providers' data-handling/retention terms are acceptable for real (even if synthetic/sample) legal documents.
